...
/**
 * Sycle_Appointments_Settings Class
 *
 * @class Sycle_Appointments_Settings
 * @version	1.0.0
 * @since 1.0.0
 * @package	Starter_Plugin
 * @author Jeffikus
 */
final class Sycle_Appointments_Settings {
	/**
	 * Sycle_Appointments_Admin The single instance of Sycle_Appointments_Admin.
	 * @var 	object
	 * @access  private
	 * @since 	1.0.0
	 */
	...
	/**
	 * Main Sycle_Appointments_Settings Instance
	 *
	 * Ensures only one instance of Sycle_Appointments_Settings is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 * @static
	 * @return Main Sycle_Appointments_Settings instance
	 */
		...
		if ( is_array( $input ) && 0 < count( $input ) ) {
			...
			foreach ( $input as $k => $v ) {
				...
				if ( ! method_exists( $this, $method ) ) {
					if ( true === (bool)apply_filters( 'sycle-appointments-validate-field-' . $fields[$k]['type'] . '_use_default', true ) ) {
				...
				if ( '' != $method ) {
					add_filter( 'sycle-appointments-validate-field-' . $fields[$k]['type'], array( $this, $method ) );
				}

				$method_output = apply_filters( 'sycle-appointments-validate-field-' . $fields[$k]['type'], $v, $fields[$k] );
	...
	public function render_field ( $args ) {
		...
		// Construct the key.
		$key 				= Sycle_Appointments()->token . '-' . $args['section'] . '[' . $args['id'] . ']';
		...
		if ( isset( $args['type'] ) && ! in_array( $args['type'], (array)apply_filters( 'sycle-appointments-no-description-fields', array( 'checkbox' ) ) ) ) {
			if ( isset( $args['description'] ) ) {
				$description = '<p class="description">' . wp_kses_post( $args['description'] ) . '</p>' . "\n";
				if ( in_array( $args['type'], (array)apply_filters( 'sycle-appointments-new-line-description-fields', array( 'textarea', 'select' ) ) ) ) {
	...
	public function get_settings_sections () {
		...
		$settings_sections['standard-fields'] = __( 'API Settings', 'sycle-appointments' );
		//$settings_sections['special-fields'] = __( 'Special Fields', 'sycle-appointments' );
		...
		return (array)apply_filters( 'sycle-appointments-settings-sections', $settings_sections );
	...
	public function get_settings_fields ( $section ) {
		...
		switch ( $section ) {
			...
			$settings_fields['sycle_username'] = array(
				'name' => __( 'Sycle Username', 'sycle-appointments' ),
				'type' => 'text',
				'default' => '',
				'section' => 'standard-fields',
				'description' => __( 'Your Sycle Username.', 'sycle-appointments' )
			);

			$settings_fields['sycle_pw'] = array(
				'name' => __( 'Sycle Password', 'sycle-appointments' ),
				'type' => 'text',
				'default' => '',
				'section' => 'standard-fields',
			...
			$settings_fields['logging'] = array(
				...
				'default' => '1',
				'section' => 'standard-fields',
				'description' => __( 'Turn on logging.', 'sycle-appointments' )
			);

			$settings_fields['cleanondeactivate'] = array(
				'name' => __( 'Remove all data', 'sycle-appointments' ),
				'type' => 'checkbox',
				'default' => '',
				'section' => 'standard-fields',
				'description' => __( 'Remove all data on deactivation of this plugin.', 'sycle-appointments' )
			);
/*
			$settings_fields['textarea'] = array(
				'name' => __( 'Example Textarea', 'sycle-appointments' ),
				'type' => 'textarea',
				'default' => '',
				'section' => 'standard-fields',
				'description' => __( 'Place the field description text here.', 'sycle-appointments' )
			);
			$settings_fields['checkbox'] = array(
				'name' => __( 'Example Checkbox', 'sycle-appointments' ),
				'type' => 'checkbox',
				'default' => '',
				'section' => 'standard-fields',
				'description' => __( 'Place the field description text here.', 'sycle-appointments' )
			...
			$settings_fields['radio'] = array(
				'name' => __( 'Example Radio Buttons', 'sycle-appointments' ),
				'type' => 'radio',
				'default' => '',
				'section' => 'standard-fields',
				'options' => array(
					'one' => __( 'One', 'sycle-appointments' ),
					'two' => __( 'Two', 'sycle-appointments' ),
					'three' => __( 'Three', 'sycle-appointments' )
				),
				'description' => __( 'Place the field description text here.', 'sycle-appointments' )
			);
			$settings_fields['select'] = array(
				'name' => __( 'Example Select', 'sycle-appointments' ),
				'type' => 'select',
				'default' => '',
				'section' => 'standard-fields',
				'options' => array(
...
/*
			...
			$settings_fields['select_taxonomy'] = array(
				'name' => __( 'Example Taxonomy Selector', 'sycle-appointments' ),
...
*/
		...
		return (array)apply_filters( 'sycle-appointments-settings-fields', $settings_fields );
	...
	public function get_supported_fields () {
		return (array)apply_filters( 'sycle-appointments-supported-fields', array( 'text', 'checkbox', 'radio', 'textarea', 'select', 'select_taxonomy' ) );
	...
	public function get_value ( $key, $default, $section ) {
		$values = get_option( 'sycle-appointments-' . $section, array() );
	...
	public function get_settings ( $section = '' ) {
		...
		if ( 0 < count( $sections ) ) {
			foreach ( $sections as $k => $v ) {
				...
				$values = get_option( 'sycle-appointments-' . $v, array() );
